# resnet_validate.py
# Validate the saved dg bundle in ./resnet/ (resnet18.pt, resnet18.py,
# config.json, schema.json) by reloading the model and measuring its accuracy
# on the CIFAR-10 test set.
#
# The bundle uses resnet18.* names (not dg's default model.pt/model.py), so this
# reproduces what dg.from_pretrained does, against the renamed files.
#
#     ../pyenv/bin/python resnet_validate.py        # full test set
#     MAX_STEPS=20 ../pyenv/bin/python resnet_validate.py   # quick check
...
BUNDLE = os.path.join(HERE, "resnet")
DATA_DIR = os.path.join(HERE, "data")


def load_bundle(bundle):
    """Reload a model from a dg bundle that uses resnet18.* file names."""
    src = os.path.join(bundle, "resnet18.py")
    weights = os.path.join(bundle, "resnet18.pt")
    ...
    # import resnet18.py as a throwaway module to get the Model class
    spec = importlib.util.spec_from_file_location("_resnet_bundle", src)
