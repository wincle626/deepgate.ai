"""Export the trained `wakeword` model to a full-integer (int8) .tflite file.

Dedicated to the artifact in ``train/trained/`` (saved by ``train.py`` via
``model.save_pretrained``); writes ``train/trained/model.tflite``. Activations
are calibrated on the real MFCC features cached during training. Paths are
resolved relative to this file, so it can be run from anywhere:

    ./pyenv/bin/python train/export_tflite.py
...
SRC_DIR = os.path.join(HERE, "trained")                  # save_pretrained artifact
OUT_PATH = os.path.join(SRC_DIR, "model.tflite")
CACHE = os.path.join(HERE, "data", "speech_commands_cache")
REP_X = os.path.join(CACHE, "testing_x.pt")              # calibration / eval features
REP_Y = os.path.join(CACHE, "testing_y.pt")
...
def _real_features():
    """Load cached MFCC features (N,1,49,10 NCHW) -> NHWC float32."""
    if not os.path.exists(REP_X):
        raise FileNotFoundError(
            f"{REP_X} not found. Run train.py first to build the feature cache."
        )
    obj = torch.load(REP_X, weights_only=True)
    if not torch.is_tensor(obj) or obj.dim() != 4:
        raise ValueError(f"{REP_X} is not a (N,1,49,10) feature tensor.")
    return obj.float().numpy().transpose(0, 2, 3, 1)     # -> (N, 49, 10, 1)


def representative_dataset():
    feats = _real_features()
    idx = np.random.default_rng(0).permutation(len(feats))[:300]
    for i in idx:
        yield [feats[i:i + 1].astype(np.float32)]
...
def validate(keras_model, tflite_bytes, torch_model):
    x = _real_features()[:1].astype(np.float32)          # a real sample
    with torch.no_grad():
...
def tflite_accuracy(tflite_bytes, n=2000):
    """Top-1 accuracy of the int8 tflite over cached test features."""
    if not os.path.exists(REP_Y):
        return
    feats = _real_features()
    labels = torch.load(REP_Y, weights_only=True).numpy()
    interp = tf.lite.Interpreter(model_content=tflite_bytes)
    interp.allocate_tensors()
    inp_d, out_d = interp.get_input_details()[0], interp.get_output_details()[0]
    s, zp = inp_d["quantization"]
    ...
    correct = 0
    for i in range(n):
        xq = np.clip(np.round(feats[i:i + 1] / s + zp), -128, 127).astype(np.int8)
        interp.set_tensor(inp_d["index"], xq)
        interp.invoke()
        if int(interp.get_tensor(out_d["index"])[0].argmax()) == int(labels[i]):
            correct += 1
    print(f"tflite top-1 over {n} samples: {100 * correct / n:.2f}%")
